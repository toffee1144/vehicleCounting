...
import time
import cv2
from detection import HailoDetectionApp, run_hls_pipeline  # Your detection code
...
def main():
    ...
    # Start the HLS pipeline thread.
    hls_thread = threading.Thread(target=run_hls_pipeline, daemon=True)
    hls_thread.start()
    ...
    mqtt_thread = threading.Thread(target=send_data_via_mqtt, args=(hailo_app,), daemon=True)
    mqtt_thread.start()
